    ...
    partial class BancoInter : IBancoCNAB400
    {
        ...
        public void LerDetalheRetornoCNAB400Segmento1(ref Boleto boleto, string registro)
        {
            try
            {
                ...
                boleto.Carteira = registro.Substring(86, 3).PadLeft(2, '0');
                boleto.TipoCarteira = TipoCarteira.CarteiraCobrancaSimples;

                //Identificação do Título no Banco
                boleto.NossoNumero = registro.Substring(70, 11); //Sem o DV
                boleto.NossoNumeroDV = "0"; //DV
                ...
                boleto.EspecieDocumento = AjustaEspecieCnab400(registro.Substring(144, 2));
        ...
        private string GerarDetalheRemessaCNAB400Registro1(Boleto boleto, ref int numeroRegistroGeral)
        {
            try
            {
                ...
                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0021, 003, 0, boleto.Carteira, '0');
                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0024, 004, 0, boleto.Banco.Beneficiario.ContaBancaria.Agencia, '0');
                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0028, 010, 0, boleto.Banco.Beneficiario.ContaBancaria.Conta, '0');
                ...
                else
                {
                    ...
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0084, 006, 0, "0", '0');
                }
                ...
                else
                {
                    ...
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0178, 006, 0, "0", ' ');
                }

                if (boleto.ValorDesconto == 0)
                {
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0184, 001, 0, "0", '0'); // Sem Desconto
                }
                else
                {
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0184, 001, 0, "1", '0'); // Sem Desconto
                }

                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0185, 013, 2, boleto.ValorDesconto, '0');
                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0198, 004, 0, "0", '0'); // Percentual Desconto
                if (boleto.ValorDesconto == 0)
                {
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0202, 006, 0, "0", '0'); // Data Desconto
                }
                else
                {
                    reg.Adicionar(TTiposDadoEDI.ediDataDDMMAA___________, 0202, 006, 0, boleto.DataDesconto, '0'); // Com Desconto
                }

                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0208, 013, 2, boleto.ValorAbatimento, '0');
                ...
                reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0317, 005, 0, boleto.Pagador.Endereco.CEP.Substring(0, 5), ' ');
                reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0322, 003, 0, boleto.Pagador.Endereco.CEP.Replace("-", "").Substring(5, 3), ' ');
                if (IsNullOrEmpty(boleto.Avalista.Nome))
                {
                    // Não tem avalista.
                    reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0325, 070, 0, Empty, ' ');
                }
                else if (boleto.Avalista.TipoCPFCNPJ("A") == "F")
                {
                    // Avalista Pessoa Física
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0325, 009, 0, boleto.Avalista.CPFCNPJ.Substring(0, 9), '0');
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0334, 004, 0, "0", '0');
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0338, 002, 0, boleto.Avalista.CPFCNPJ.Substring(9, 2), '0');
                    reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0340, 002, 0, Empty, ' ');
                    reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0342, 053, 0, boleto.Avalista.Nome, ' ');
                }
                else
                {
                    // Avalista Pessoa Juridica
                    reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0325, 015, 0, boleto.Avalista.CPFCNPJ, '0');
                    reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0340, 002, 0, Empty, ' ');
                    reg.Adicionar(TTiposDadoEDI.ediAlphaAliEsquerda_____, 0342, 053, 0, boleto.Avalista.Nome, '0');
                }
                reg.Adicionar(TTiposDadoEDI.ediNumericoSemSeparador_, 0395, 006, 0, numeroRegistroGeral, '0');
                reg.CodificarLinha();
                return reg.LinhaRegistro;
